...
def _parse_nodes(custom_action_param: str) -> list[str]:
    ...
    if not isinstance(params, dict):
        raise ValueError(
            f"RealTimeTaskAction: invalid JSON object: {custom_action_param}"
        )

    nodes = params.get("nodes")
    if not isinstance(nodes, list) or len(nodes) == 0:
        raise ValueError(
            f"RealTimeTaskAction: 'nodes' missing, not an array, or empty: {custom_action_param}"
        )

    for v in nodes:
        if not isinstance(v, str):
            raise ValueError(
                "RealTimeTaskAction: every entry in 'nodes' must be a string"
            )
