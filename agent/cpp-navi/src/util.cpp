...
std::optional<fs::path> find_pktmon_backend()
{
    std::vector<fs::path> candidates {
        executable_dir() / "pktmon_backend.dll",
        project_root() / ".venv" / "Lib" / "site-packages" / "pktmon_interface" / "pktmon_backend.dll",
        project_root() / "python" / "Lib" / "site-packages" / "pktmon_interface" / "pktmon_backend.dll",
        executable_dir().parent_path() / "python" / "Lib" / "site-packages" / "pktmon_interface" / "pktmon_backend.dll",
    };
    if (const char* configured = std::getenv("PKTMON_BACKEND_DLL")) {
        candidates.insert(candidates.begin(), fs::path(configured));
    }
    const auto found = std::ranges::find_if(candidates, [](const fs::path& path) { return fs::exists(path); });
    return found == candidates.end() ? std::nullopt : std::optional<fs::path>(*found);
}
