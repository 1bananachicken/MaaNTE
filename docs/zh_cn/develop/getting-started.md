...
三种选项类型：`switch`（开关）、`input`（数值输入）、`select`（下拉选择）。详见 [任务配置](./task-config.md)。
...
五种语言文件：`zh_cn.json`、`zh_tw.json`、`en_us.json`、`ja_jp.json`、`ko_kr.json`。详见 [i18n](./i18n.md)。
