...
网络坐标的抓包和解析继续由 PyArmor 保护的 `thirdparty/nte_coordinate_api*.pyd` 提供。Python Agent 通过仅监听 `127.0.0.1` 的 bridge 向 C++ Navi Agent 返回解析后的姿态；C++ 源码不包含包格式或解析算法。受保护模块会依次尝试 Npcap 和 `pktmon-interface` 后端，pktmon 通常需要管理员权限。
